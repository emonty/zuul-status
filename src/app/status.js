...
function format_progress(elapsed, remaining) {
    ...
    var r = '<progress class="change_progress" title="' +
...
function format_change(change, change_queue) {
    ...
    var display = $('#expandByDefault').is(':checked');
    var safe_change_id = safe_id(change.id);
    var collapsed_index = window.zuul_collapsed_exceptions.indexOf(safe_change_id);
...
function toggle_display_jobs(e, _header) {
    ...
    content.slideToggle(100, function () {
        var changeid = header.parent().attr('id');
        var collapsed_index = window.zuul_collapsed_exceptions.indexOf(changeid);
        var expand_by_default = $('#expandByDefault').is(':checked');
...
function clean_changes_lists() {
    var new_collapsed_exceptions = [];

    $.each($('div.change'), function(i, change) {
        var collapsed_index = window.zuul_collapsed_exceptions.indexOf(change.id);
        if (collapsed_index > -1) {
